...
function formatRevenue(revenue) {
  ...
  // Fix: only use K/M suffix when the value is actually >= 1000/1000000
  if (v >= 1000000) return `KSh ${(v / 1000000).toFixed(1)}M`;
  if (v >= 1000) return `KSh ${(v / 1000).toFixed(1)}K`;
    ...
    <div>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        <StatCard
          ...
          sub={`+${safeStats.newUsersThisWeek || 0} this week`}
          icon="👥"
          color="bg-blue-50"
          change={12}
        />
        <StatCard
          ...
          sub={`${safeStats.pendingOrders || 0} pending`}
          icon="🛒"
          color="bg-orange-50"
          change={8}
        />
        ...
        <StatCard
          ...
          value={(safeStats.activeProducts || 0).toLocaleString()}
          sub={`${safeStats.products || 0} total listed`}
          icon="🌾"
          color="bg-green-50"
          change={5}
        />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        <div className="bg-white rounded-3xl p-8 border border-[#E8EEE5] shadow-sm">
          ...
          <RoleBreakdown roleBreakdown={safeRoleBreakdown} />
        </div>

        <div className="bg-white rounded-3xl p-8 border border-[#E8EEE5] shadow-sm">
          <h3 className="font-black text-lg mb-6">🛒 Recent Orders</h3>
          <RecentOrders recentOrders={safeRecentOrders} />
        </div>
      </div>
        ...
        {[
          {
            ...
            value: safeStats.activeDeliveries || 0,
            icon: "🚚",
            color: "bg-purple-50",
          },
          {
            label: "Total Deliveries",
            value: safeStats.deliveries || 0,
            icon: "📦",
            color: "bg-indigo-50",
          },
          {
          ...
          },
          {
            ...
            value: safeStats.newUsersThisWeek || 0,
            icon: "🆕",
            color: "bg-pink-50",
          },
        ].map((s) => (
      ...
      {/* Recent users */}
      <RecentUsers recentUsers={safeRecentUsers} onViewAll={onViewAllUsers} />
    </div>
  );
}
